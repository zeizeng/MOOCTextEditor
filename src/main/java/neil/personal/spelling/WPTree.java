...
public class WPTree implements WordPath {
    ...
    public List<String> findPath(String word1, String word2) {
        ...
        queue.add(root);
        String findWord = "";
        WPTreeNode needWpTreeNode = null;
        while (!queue.isEmpty() && !findWord.equals(word2)) {
            WPTreeNode wpTreeNode = queue.remove(0);
            List<String> nearby = nw.distanceOne(wpTreeNode.getWord(), true);
            for (String near : nearby) {
                if (near.equals(word2)) {
                    findWord = near;
                    needWpTreeNode = new WPTreeNode(near, wpTreeNode);
                    break;
                } else if (!visited.contains(near)) {
                    wpTreeNode.addChild(near);
                    visited.add(near);
                    queue.add(new WPTreeNode(near, wpTreeNode));
                }
            }
        }
        if (needWpTreeNode == null) return null;

        return needWpTreeNode.buildPathToRoot();
    }
